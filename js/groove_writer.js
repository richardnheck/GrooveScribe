...
function GrooveWriter() {
  ...
  function get_snare_state(id, returnType) {
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Flam;
      // snare flam
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Drag;
      // snare drag
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Ghost;
      // ghost note
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Accent;
      // snare accent
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Normal;
      // snare normal
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_XStick;
      // snare Xstick
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Buzz;
      // snare Buzz
  ...
  function get_kick_state(id, returnType) {
    ...
    if (splashOn && kickOn) {
      if (returnType == "ABC") return constant_ABC_KI_SandK;
      // kick & splash
      else if (returnType == "URL") return "X"; // kick & splash
    } else if (splashOn) {
      if (returnType == "ABC") return constant_ABC_KI_Splash;
      // splash only
      else if (returnType == "URL") return "x"; // splash only
    } else if (kickOn) {
      if (returnType == "ABC") return constant_ABC_KI_Normal;
      // kick normal
  ...
  function get_hh_state(id, returnType) {
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Ride;
      // ride
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Ride_Bell;
      // ride bell
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Cow_Bell;
      // cow bell
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Crash;
      // crash
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Stacker;
      // stacker
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Metronome_Normal;
      // beep
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Metronome_Accent;
      // beep
      else if (returnType == "URL") return "N"; // beep
    }
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Open;
      // hh Open
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Close;
      // hh close
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Accent;
      // hh accent
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Normal;
      // hh normal
  ...
  function get_sticking_state(id, returnType) {
    ...
    if (both_ele.style.color == constant_sticking_both_on_color_rgb) {
      ...
      if (returnType == "ABC") return constant_ABC_STICK_OFF;
      // off (rest)
      else if (returnType == "URL") return "-"; // off (rest)
    }
  ...
  function set_counting_state(id, new_state, make_sound) {
    // turn both off
    document.getElementById("counting_count" + id).style.color =
      constant_note_hidden_color_rgb;
    ...
    switch (new_state) {
      ...
      case "count":
        ...
        document.getElementById("counting_count" + id).style.color =
          constant_sticking_count_on_color_rgb;
        document.getElementById("counting_count" + id).innerHTML =
          "" + count_state;
        break;
  ...
  function hilight_individual_note(instrument, id) {
    ...
    if (
      ...
      if (
        class_cur_tom2_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "tom2-" + class_cur_tom4_highlight_id
        ).style.borderColor = "transparent";
      class_cur_tom2_highlight_id = false;
    }
    if (
  ...
  function clear_all_highlights(instrument) {
    ...
    if (class_cur_tom2_highlight_id !== false) {
      document.getElementById(
        "tom2-" + class_cur_tom2_highlight_id
      ).style.borderColor = "transparent";
      class_cur_tom2_highlight_id = false;
    }
  ...
  function get32NoteArrayFromClickableUI(
    ...
    for (var i = 0; i < class_notes_per_measure; i++) {
      ...
      if (isCountingsVisible())
        Counting_Array[array_index] = get_counting_state(
          i + startIndexForClickableUI,
          "ABC"
        );
      ...
      if (isTomsVisible()) {
        ...
        Toms_Array[1][array_index] = get_tom_state(
          i + startIndexForClickableUI,
          2,
          "ABC"
        );
  ...
  function set_Default_notes(encodedURLData) {
    ...
    if (myGrooveData.showStickings) root.showHideStickings(true, true, true);

    if (myGrooveData.showCountings) root.showHideCountings(true, true, true);
  ...
  function changeDivisionWithNotes(
    ...
    if (wasStickingsVisable) root.showHideStickings(true, true, true);

    if (wasCountingsVisable) root.showHideCountings(true, true, true);
  ...
  root.changeDivision = function (newDivision) {
    ...
    if (usingTriplets() === isNewDivisionTriplets) {
      ...
      uiCountings = root.myGrooveUtils.GetDefaultStickingsGroove(
        new_notes_per_measure,
        class_num_beats_per_measure,
        class_note_value_per_measure,
        class_number_of_measures
  ...
  root.HTMLforStaffContainer = function (baseindex, indexStartForNotes) {
      ...
      '\
							<div class="countings-row-container">\
								<div class="line-labels">\
									<div class="countings-label" onClick="myGrooveWriter.noteLabelClick(event, \'countings\', ' +
      baseindex +
      ")\" oncontextmenu=\"event.preventDefault(); myGrooveWriter.noteLabelClick(event, 'countings', " +
      baseindex +
      ')">COUNTINGS</div>\
								</div>\
    ...
    for (
      ...
      newHTML +=
        ...
        i +
        '" class="counting">\n\
															<div class="counting_count note_part"   id="counting_count' +
        i +
        "\"   onClick=\"myGrooveWriter.noteLeftClick(event, 'counting', " +
        i +
        ")\" oncontextmenu=\"event.preventDefault(); myGrooveWriter.noteRightClick(event, 'counting', " +
        i +
        ')">C</div>\n\
														</div>\n\
													';
    ...
    for (
      ...
      newHTML +=
        '						<div id="bg-highlight' + i + '" class="bg-highlight" >\
												</div>\n';
