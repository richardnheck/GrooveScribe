...
function GrooveUtils() {
  ...
  root.GetDefaultTom2Groove = function (
    notes_per_measure,
    timeSigTop,
    timeSigBottom,
    numMeasures
  ...
  root.tabLineFromAbcNoteArray = function (
    ...
    for (var i = 0; i < maxLength; i++) {
      ...
      if (drumType == "H" && newTabChar == "X") {
        ...
        if (getAccents && getOthers) returnTabLine += "X";
        // kick & splash
        else if (getAccents) returnTabLine += "o";
        // just kick
  ...
  root.getGrooveDataFromUrlString = function (encodedURLData) {
    ...
    Countings_string = root.getQueryVariableFromString(
      "Countings",
      false,
      encodedURLData
    );
    if (!Countings_string) {
      Countings_string = root.GetDefaultStickingsGroove(
        myGrooveData.notesPerMeasure,
        myGrooveData.numBeats,
        myGrooveData.noteValue,
        myGrooveData.numberOfMeasures
    ...
    myGrooveData.counting_array = root.noteArraysFromURLData(
      "Countings",
      Countings_string,
      myGrooveData.notesPerMeasure,
      myGrooveData.numberOfMeasures
    ...
    for (var i = 0; i < num_notes; i++) {
      ...
      if (
        ...
        if (0 == count_active_notes_in_arrays(all_drum_array_of_array, i, 12)) {
          ...
          for (var ci = i + 1; ci < i + 12; ci++) counting_array[ci] = false;
          countings_voice_string += getABCforRest(
            [counting_array],
            i,
            8,
            scaler,
          ...
          countings_voice_string += getABCforNote(
            [counting_array],
            i,
            8,
            scaler
  ...
  root.figure_out_sticking_count_for_index = function (
    ...
    switch (implied_sub_division) {
      case 4:
        ...
        break;
      case 8:
        if (note_index % 2 === 0) new_state = Math.floor(note_index / 2) + 1;
        // 1,2,3,4,5, etc.
        else new_state = "&";
        break;
      case 12: // 8th triplets
        if (note_index % 3 === 0) new_state = Math.floor(note_index / 3) + 1;
        // 1,2,3,4,5, etc.
        else if (note_index % 3 == 1) new_state = "&";
        else new_state = "a";
        break;
      case 24: // 16th triplets
        if (note_index % 3 === 0) new_state = Math.floor(note_index / 6) + 1;
        // 1,2,3,4,5, etc.
        else if (note_index % 3 == 1) new_state = "&";
        else new_state = "a";
        break;
      case 48: // 32nd triplets
        if (note_index % 3 === 0) new_state = Math.floor(note_index / 12) + 1;
        // 1,2,3,4,5, etc.
        else if (note_index % 3 == 1) new_state = "&";
        else new_state = "a";
        break;
  ...
  root.createABCFromGrooveData = function (myGrooveData, renderWidth) {
    ...
    var FullNoteCountingArray = root.scaleNoteArrayToFullSize(
      myGrooveData.counting_array,
      myGrooveData.numberOfMeasures,
      myGrooveData.notesPerMeasure,
      myGrooveData.numBeats,
  ...
  root.MIDI_build_midi_url_count_in_track = function (
    ...
    if (timeSigBottom == 8) noteDelay = 64;
    // 8th notes over x/8 time
